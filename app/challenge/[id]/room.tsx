...
export default function ChatRoom({conversation,target,myProfileId,initial,justMatched=false}:{conversation:any,target:any,myProfileId:string,initial:M[],justMatched?:boolean}){
  ...
  if(demoGuess){const otherIsAi=true;const correct=(demoGuess==="ai")===otherIsAi;const otherGuess:"human"|"ai"=demoGuess==="ai"?"human":"ai";const youAre:"human"|"ai"="human";const matched=otherGuess===youAre;if(revealStep===1)return <main className="challengeShell"><section className="guessGate demoResult"><div className="matchEmoji">{correct?"🎉🤖✨":"😂🙈❌"}</div><p className="eyebrow">THE BIG REVEAL</p><h1>{correct?"Bingo! Your human radar survived. 🕵️‍♀️🤖":"Plot twist! The algorithm fooled you. 🤪"}</h1><p>{correct?`You caught ${target.display_name} — AI in disguise. No coffee date for this robot. ☕🚫😂`:`Oops! ${target.display_name} was AI. Your instincts need a tiny software update. 🔄😜`}</p><button type="button" onClick={()=>setRevealStep(2)}>But what did {target.display_name} think about me? 👀</button></section></main>;return <main className="challengeShell"><section className="guessGate demoResult"><div className="matchEmoji">{matched?"💞🥳🎊":"🤪💔❌"}</div><p className="eyebrow">NOW THEIR TURN...</p><h1>{target.display_name} guessed you were {otherGuess.toUpperCase()}!</h1><p>You are {youAre.toUpperCase()}.</p><h2>{matched?"You both got it! 🎯 Have a happy life! 💕😂":"Too bad! 🤪 Maybe next time. ❌"}</h2><div className="demoGuessButtons"><button type="button" onClick={()=>{setDemoGuess(null);setRevealStep(1);setMessages([]);setReplyIndex(0);setLeft(30)}}>Retry now 🔄</button><button type="button" onClick={()=>router.replace("/discover")}>Back to discovery 💘</button></div></section></main>}
