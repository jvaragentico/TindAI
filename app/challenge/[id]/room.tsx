...
export default function ChatRoom({conversation,target,myProfileId,initial,justMatched=false}:{conversation:any,target:any,myProfileId:string,initial:M[],justMatched?:boolean}){
  ...
  if(demoGuess){
    if(revealStep===1)return <main className="challengeShell"><section className="guessGate demoResult"><div className="matchEmoji">{demoGuess==="ai"?"🎉🤖✨":"😂🙈❌"}</div><p className="eyebrow">THE BIG REVEAL</p><h1>{demoGuess==="ai"?"Bingo! Your human radar survived. 🕵️‍♀️🤖":"Plot twist! The algorithm fooled you. 🤪"}</h1><p>{demoGuess==="ai"?"You caught the AI in disguise. No coffee date for this robot. ☕🚫😂":"Oops! Your instincts need a tiny software update. 🔄😜"}</p><button type="button" onClick={()=>setRevealStep(2)}>What did they think about me? 👀</button></section></main>;
    return <main className="challengeShell"><section className="guessGate demoResult"><div className="matchEmoji">{demoGuess==="ai"?"💞🥳🎊":"🤪💔❌"}</div><p className="eyebrow">NOW THEIR TURN...</p><h1>They guessed you were {demoGuess==="ai"?"HUMAN":"AI"}!</h1><p>You are HUMAN.</p><h2>{demoGuess==="ai"?"You both got it! 🎯 Have a happy life! 💕😂":"Too bad! 🤪 Maybe next time. ❌"}</h2><div className="demoGuessButtons"><button type="button" onClick={()=>{setDemoGuess(null);setRevealStep(1);setMessages([]);setReplyIndex(0);setLeft(30)}}>Retry now 🔄</button><button type="button" onClick={()=>router.replace("/discover")}>Back to discovery 💘</button></div></section></main>;
  }
